...
useEffect(() => {
  if (linkedRfp) {
    Near.asyncView("${REPL_AI_PGF_FORUM_CONTRACT}", "get_rfp", {
...
useEffect(() => {
  if (
    ...
    editProposalData.snapshot.linked_proposals.map((item) => {
      useCache(
        () =>
          Near.asyncView("${REPL_AI_PGF_FORUM_CONTRACT}", "get_proposal", {
...
const DescriptionComponent = useMemo(() => {
  return (
    <Widget
      src={`${REPL_AI_PGF_FORUM}/widget/components.molecule.Compose`}
      props={{
        data: description,
...
const LinkRFPComponent = useMemo(() => {
  return (
    <div className="d-flex flex-column gap-1">
      <Widget
        src={`${REPL_AI_PGF_FORUM}/widget/components.molecule.LinkedRfpDropdown`}
        props={{
...
const LinkedProposalsComponent = useMemo(() => {
  return (
    <div className="d-flex flex-column gap-1">
      ...
      <Widget
        src={`${REPL_AI_PGF_FORUM}/widget/components.molecule.LinkedProposalsDropdown`}
        props={{
...
return (
  <Container className="w-100 py-4 px-0 px-sm-2 d-flex flex-column gap-3">
    ...
    <Widget
      src={`${REPL_AI_PGF_FORUM}/widget/components.proposals.ViewProposalModal`}
      props={{
